  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className="text-xs text-gray-400 mb-1 block">Title</label><input className="input" value={form.title} onChange={e => set('title', e.target.value)} required /></div>
        <div><label className="text-xs text-gray-400 mb-1 block">Type</label><input className="input" value={form.type} onChange={e => set('type', e.target.value)} placeholder="e.g. Functional, Cognitive" /></div>
        <div><label className="text-xs text-gray-400 mb-1 block">Max Score</label><input className="input" type="number" value={form.max_score} onChange={e => set('max_score', e.target.value)} /></div>
        <div><label className="text-xs text-gray-400 mb-1 block">Scheduled Date</label><input className="input" type="datetime-local" value={form.scheduled_date} onChange={e => set('scheduled_date', e.target.value)} /></div>
        <div className="col-span-2"><label className="text-xs text-gray-400 mb-1 block">Notes</label><textarea className="input" rows={3} value={form.notes} onChange={e => set('notes', e.target.value)} /></div>
      </div>
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="p-3 bg-dark-700/50 rounded-lg text-sm">
        ...
        <p className="text-gray-400">{assessment.patient_name} · {assessment.doctor_name}</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label className="text-xs text-gray-400 mb-1 block">Score (/{assessment.max_score})</label><input className="input" type="number" value={form.score} onChange={e => set('score', e.target.value)} /></div>
        <div><label className="text-xs text-gray-400 mb-1 block">Completed Date</label><input className="input" type="datetime-local" value={form.completed_date} onChange={e => set('completed_date', e.target.value)} /></div>
      </div>
      <div><label className="text-xs text-gray-400 mb-1 block">Notes</label><textarea className="input" rows={2} value={form.notes} onChange={e => set('notes', e.target.value)} /></div>
      <div><label className="text-xs text-gray-400 mb-1 block">Protocol Improvement Notes</label><textarea className="input" rows={2} value={form.improvement_notes} onChange={e => set('improvement_notes', e.target.value)} /></div>
      <div className="flex gap-3">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="card p-4">
            <h3 className="font-semibold text-white mb-3">Upcoming Assessments</h3>
            <div className="space-y-3">
              {upcoming.length === 0 ? <EmptyState message="No upcoming assessments" /> : upcoming.map(a => (
                <div key={a.id} className="p-3 bg-dark-700/50 rounded-lg">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-white">{a.title}</p>
                      <p className="text-xs text-gray-400">{a.patient_name}</p>
                      {a.scheduled_date && <p className="text-xs text-gray-500 mt-1">{format(new Date(a.scheduled_date), 'MMM d, h:mm a')}</p>}
                    </div>
          ...
          <div className="lg:col-span-2 card">
            <div className="flex items-center justify-between p-4 border-b border-dark-700">
              <h3 className="font-semibold text-white">All Assessments</h3>
                ...
                <tbody>
                  {assessments.length === 0 ? (
                    ...
                    <tr key={a.id} className="table-row">
                      <td className="table-cell font-medium text-white">{a.title}</td>
                      <td className="table-cell">{a.patient_name}</td>
                      <td className="table-cell">{a.doctor_name || '-'}</td>
                      <td className="table-cell">{a.type || '-'}</td>
                      <td className="table-cell">{a.score ? `${a.score}/${a.max_score}` : '-'}</td>
                      <td className="table-cell text-xs">{a.scheduled_date ? format(new Date(a.scheduled_date), 'MMM d') : '-'}</td>
                      <td className="table-cell"><StatusBadge status={a.status} /></td>
                      <td className="table-cell">
                        <button onClick={() => { setSelected(a); setModal('update'); }} className="text-xs text-primary-400 hover:text-primary-300">Update</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
